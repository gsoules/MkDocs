...
-   Describes how a Digital Archive site administrator defines relationship types
...
to John. The *married to* type accurately describes the relationship in either direction.
...
relationship’s type ensure correct direction when an archivist adds a relationship.
...
To relate family members, an archivist only ever has to choose *one* of the relationships below to indicate how
...
!!! note ""
    The following material may be difficult to follow. If you need help creating or editing relationship
    types, seek help from someone with experience.
...
The types appear on the page in the order in which you want users to see when viewing an item.
...
Add
:   Click the **_Add Relationship Type_** button located at the bottom of the the page
    (not shown in the screenshot above).

Edit
:   Click the arrow icon to the far right of the type. This expands the row to let you make changes.

Remove
:   Click the arrow icon to the far right of the type, then click the **_Remove_** button. The button
    only appears if the type is not in use. You cannot remove a type that is in use until you either
    change all the items that are using it to use a different relationship type, or you remove the
    relationship type from all items that are using it.

## Edit a relationship

### Bi-directional type

Let’s start with a simple example by editing the *married to* relationship type.
...
the screenshot below which shows the specification of the resided at relationship. Note also that the
terms source and target are used only when editing a relationship and at no other time. You only need
to fully understand this if you will be responsible for adding or updating relationship types.

### Uni-directional type

A relationship type that is uni-direction uses separate *labels* for its source and target. In this
example we'll edit the *wrote / written by* relationship type which is just one type, but with two
different labels *wrote* and *written by*. Note that from an archivist's perspective, this may seem
like two different relationships, but that's okay, because they don't need to understand or care about
how it works at this level.

![The wrote - written by relationship](relationship-types-5.jpg)

The specification above says that the source item must be a person that wrote a document or publication.
In the inverse direction, the target item must be a document or publication written by a person.
...
and the **_Target Name_** are what appear in the relationship type dropdown list used when you add a
relationship between two items. The dropdown list displays the source and target names of every
relationship type. For example, one entry in the list is *wrote*, another is *written by* and another
is *married to*.
...
[viewing related items](/user/viewing-related-items/).
The labels also appear on the arrows in visualization graphs. In the example above, except for the
letter casing, the source and names are the same as the labels. However, for other types they are
different. For example in the *resided at / occuppied by* type, the source label is also `Resided at`,
but the target label is `Residents,Resident`.

By default, the label is used for any number of items
having this relationship type; however, if English grammar dictates a different label for multiple
items than for one item, you specify the plural spelling followed by a comma and then the singular spelling.
Thus in this example, if the Digital Archive software finds more than one resident associated with an
item, the related item group will be “Residents”, but if finds only one resident the label will be “Resident”.

## Advanced Features

Most relationships types are specified using only names, labels, and rules, but to utilize the
software’s capabilities for displaying indirect relationships, and genealogies, you need to specify
Directives or Ancestry values for certain relationships.

### Directives

The Directives value lets you specify which indirect relationships the software will recognize
and display to users. The screenshot below shows Directives for the depicts relationship type.
The rest of this section uses the *depicts* type as an example, but you can specify directives
for other relationships.

The comma-separated numbers in the **_Directives_** field are Ids of these four relationship types:
...
These directives apply when viewing an item that has a *depicts* relationship to one or more items.
In the explanation that follows, each of these items is the “directly related item.” The software
applies the directives to each direct relationship as follows:
...
To better understand the algorithm above, consider item 12316 “Addison Packing Company at Southwest Harbor.”
