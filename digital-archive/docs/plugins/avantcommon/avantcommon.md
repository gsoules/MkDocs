...
AvantCommon provides no functionality by itself, but provides common logic to support the plugins listed below.
See the [Usage](#usage) section to learn how to configure this plugin.
...
Request Image URL | Provide the URL of a web page to display when a user requests an image by clicking the "Request this Image" link at the bottom of the Lightbox. When this value is empty, no "Request this Image" link appears in the Lightbox, onlye the image's file name. The link includes query string parameters for the Item #, item ID, and image file name.

#### Notes
* The Private Elements and Unused Elements options can be used together instead of using the
