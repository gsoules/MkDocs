...
## Using S3
SWHPL uses [Amazon Simple Storage Service (S3)](https://aws.amazon.com/s3/?p=ft&c=st&z=3)
to store its archival digital files in the cloud. It also uses the [AvantS3](/plugins/avants3/) plugin which integrates S3 with
the Digital Archive. While S3 is not as straighforward or as easy to use as other cloud storage services like Google Drive or Dropbox, it is a faster,cheaper, safer, and more powerful alternative overall.

S3 is not a traditional, folder-oriented hierarchical file system like you find on a PC or a Mac, but it can be viewed
that way and so this documentation uses the familiar word "folder" to describe where files are stored.

SWHPL's S3 storage has just two top-level folders:

-   Database folder
-   Accessions folder

### Database folder
The Database folder stores files that are associated with individual items in the Digital Archive database.
...
At the top level of the Database folder are 16 grouping subfolders named `1000`, `2000`, up to `16000`. Each of these folders
...
usually include the Word document (called a reference sheet) used to record and format the information, and a PDF
...
### Accessions folder
The Accessions folder stores files that belong to an accession or sub-accession. At the top level of the Accessions folder are
...
For example, primary accession `2026` has a top-level folder in the S3 Accessions folder and each of its sub-accessions have their
...
Which top-level S3 folder you use for storing files &mdash; Database or Accessions &mdash; depends on whether the
files are for an accession or for a new item that does not belong to an accession.

#### Accessions folder
You always create a new subfolder in the Accessions folder when adding a new accession.

#### Database folder
You create a new subfolder in the Database folder *only* when adding a new item to the Digital Archive *and* the item's
files do not come from an accession.

Here are examples of when you need to create a new subfolder in the Database folder:

-   Add a new item from files in the digital backlog which are not part of an accession.
-   Add a new Reference Item that has a PDF file.  
    The Word document and PDF would not be part of an accession.

The reason that you don't create a new subfolder Database folder for the examples above is that the item's files are
already stored in the accession folder. If you need to create additional files for the item, such as web-sized JPEGs,
you add those files to the accession folder files.

#### Special cases

By following the rules above, an item's files will either be stored within the Accessions folder or they will be stored
within the Database folder, but never in both places. However, cases may arise where an accession
contains a file that you want to attach to an existing item that is not part of the accession.

For example, while processing
a new accession you scan something that would be a perfect addition to an existing item. In that case, you can make a
web-sized JPEG of the scan and store it in the item's subfolder within the Database folder. That way, the original stays
with the accession but the JPEG gets stored with the item's other files.
